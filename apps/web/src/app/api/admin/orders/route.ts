...
export async function GET(request: Request) {
  try {
    ...
    if (
      status &&
      ["pending", "accepted", "rejected", "in_progress", "completed", "cancelled"].includes(status)
    ) {
      orders = await orderRepository
        .findAll()
        .then((all) => all.filter((order) => order.status === status));
    } else {
    ...
    const enriched = await Promise.all(
      orders.map(async (order) => {
        ...
        const craftsmanUser = order.craftsmanId
          ? await userRepository.findById(order.craftsmanId)
          : null;
        ...
        return {
          ...
          craftsman: craftsmanUser
            ? {
                id: craftsmanProfile?.id || craftsmanUser.id,
                name: craftsmanUser.name,
                craftType: craftsmanProfile?.craftType,
              }
            : null,
        };
      })
