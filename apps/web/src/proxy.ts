...
export async function proxy(request: NextRequest) {
  ...
  const onboardingComplete =
    (session?.user as { onboardingComplete?: boolean } | undefined)?.onboardingComplete ?? false;
  ...
  const onboardingRoute =
    role === "craftsman" ? "/dashboard/craftsman/onboarding" : "/choose-account";
  ...
  if (
    !session?.user &&
    (pathname.startsWith("/dashboard") || pathname.startsWith("/choose-account"))
  ) {
  ...
  if (
    session?.user &&
    onboardingComplete &&
    (pathname === "/choose-account" || pathname === "/dashboard/craftsman/onboarding")
  ) {
    return NextResponse.redirect(new URL(`/dashboard/${role}`, request.url));
  }
