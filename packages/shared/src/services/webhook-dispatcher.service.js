...
export async function enqueueOrderTimeoutCheck(orderId, timeoutAt) {
    const delay = timeoutAt.getTime() - Date.now();
    if (delay <= 0) {
        logger.warn({ orderId }, 'Order timeout already passed');
        return;
    }
    await orderTimeoutQueue.add('order:timeout-check', { orderId }, { delay, jobId: `timeout:${orderId}`, removeOnComplete: true });
    logger.info({ orderId, timeoutAt: timeoutAt.toISOString() }, 'Order timeout job enqueued');
}
export async function dispatchWebhook(event, payload, secret) {
    ...
    for (const webhook of webhooks) {
        await webhookQueue.add('webhook:send', { url: webhook.url, payload: { event, ...payload }, secret: webhook.secret }, { jobId: `webhook:${webhook.id}:${Date.now()}`, removeOnComplete: true, attempts: 3, backoff: { type: 'exponential', delay: 2000 } });
    }
    logger.info({ event, webhookCount: webhooks.length }, 'Webhooks dispatched');
}
async function findActiveWebhooksByEvent(event) {
