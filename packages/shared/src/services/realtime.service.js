 ...
 * `next/server` types.
 */
export async function publishOrderUpdate(orderId, payload = {}) {
    try {
        await valkey.publish(WS_ORDER_CHANNEL, JSON.stringify({
        ...
        logger.warn({ orderId, error: error instanceof Error ? error.message : String(error) }, 'Failed to publish order update');
    }
}
