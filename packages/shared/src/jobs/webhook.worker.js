...
export function createWebhookWorker() {
    const worker = new Worker(WEBHOOK_QUEUE_NAME, async (job) => {
        ...
        try {
            ...
            if (!response.ok) {
                ...
                throw new Error(`Webhook returned ${response.status} ${response.statusText}: ${body.slice(0, 500)}`);
            }
            logger.info({ jobId: job.id, url, event, status: response.status }, 'Webhook dispatched successfully');
        }
        finally {
    ...
    worker.on('completed', (job) => {
        logger.info({ jobId: job.id?.toString(), queue: WEBHOOK_QUEUE_NAME }, 'Webhook worker job completed');
    });
    worker.on('failed', (_job, err) => {
        logger.error({ component: 'webhook-worker', error: err.message }, 'Webhook worker job failed');
    });
    worker.on('error', (err) => {
