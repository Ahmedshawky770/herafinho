...
export function createOrderTimeoutWorker(outbox?: OutboxRepository): Worker {
  ...
  worker.on('completed', (job) => {
    logger.info({ jobId: job.id, orderId: (job.data as { orderId: string }).orderId }, 'Order timeout job completed');
  });

  worker.on('failed', (job, error) => {
    logger.error(
      { jobId: job?.id, orderId: (job?.data as { orderId?: string } | undefined)?.orderId, error: error.message },
