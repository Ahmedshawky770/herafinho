...
export class LocationRepository implements ILocationRepository {
  async upsert(
    userId: string,
    location: { latitude: string; longitude: string; isAvailable: boolean }
  ): Promise<void> {
    await db
  ...
  async getNearby(
    ...
    radiusKm: number,
    craftType?: string
  ): Promise<{ userId: string; latitude: string; longitude: string }[]> {
    const where = craftType ? eq(craftsmanLocations.userId, craftType as ID) : undefined;
    const results = await db.query.craftsmanLocations.findMany({
      where,
    });
    return results
      .filter((loc) => loc.isAvailable)
      .map(({ userId, latitude, longitude }) => ({ userId, latitude, longitude }));
  }
}
