...
describe('ReviewRepository (integration)', () => {
  ...
  it('lists reviews by craftsman', async () => {
    ...
    expect((await new ReviewRepository().findByCraftsmanId(craftsmanUser.id)).length).toBeGreaterThanOrEqual(1);
  });
  ...
  it('counts reviews by craftsman', async () => {
    ...
    expect(await new ReviewRepository().countByCraftsman(craftsmanUser.id)).toBeGreaterThanOrEqual(1);
  });
