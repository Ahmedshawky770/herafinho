...
describeIntegration("ComplaintsRepository (integration)", () => {
  ...
  it("lists complaints by reporter and by the reported user", async () => {
    ...
    expect(
      (await new ComplaintsRepository().findByReporterId(reporter.id)).length
    ).toBeGreaterThanOrEqual(1);
    expect(
      (await new ComplaintsRepository().findByAgainstUserId(against.id)).length
    ).toBeGreaterThanOrEqual(1);
  });
  ...
  it("isolation: complaints table is truncated between tests", async () => {
    // beforeEach truncates, so the table starts empty for each test.
    expect((await ctx.db.select().from(complaints)).length).toBe(0);
    await seedComplaint();
